...
import 'package:stampede/controllers/bulletin_controller.dart';
import 'package:stampede/core/utils/calendar_utils.dart';
...
enum ProfileActionType { info, link, login, calendar, sports }
...
class SettingsPage extends ConsumerWidget {
  ...
  final List<ProfileOption> _options = const [
    ProfileOption(
      title: 'Customise Sports Cards',
      subtitle: 'Choose which sports to display',
      actionType: ProfileActionType.sports,
    ),
    ProfileOption(
      title: 'Sync With Calendar',
      subtitle: 'Sync all events with your calendar',
      actionType: ProfileActionType.calendar,
    ),
    ProfileOption(
      title: 'App Info',
      subtitle: 'Version, credits, and more',
              ...
              ..._options.map((option) {
                return _buildInfoCard(
                  ...
                  onTap: () {
                    ...
                    } else if (option.actionType == ProfileActionType.calendar) {
                      final bulletinController =
                          ref.read(bulletinControllerProvider.notifier);
                      CalendarUtils.syncAllEventsToCalendar(
                          bulletinController.getFilteredPosts('All'));
                    } else if (option.actionType == ProfileActionType.sports) {
                      Get.to(() => const SportsSelectionPage());
                    }
                  },
                );
              }).toList(),
  ...
  Widget _buildProfileHeader(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        Text('Settings', style: GoogleFonts.inter(fontSize: nameFontSize, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis),
        SizedBox(height: screenWidth * 0.01),
        Text('App Preferences', style: TextStyle(fontSize: infoFontSize, color: Colors.black), maxLines: 1, overflow: TextOverflow.ellipsis),
      ],
    );
  }
